...
pdf_file = r'c:\Users\Shubham\Downloads\Balaji Udyog Decorative Glassware.pdf'
out_dir = r'c:\Users\Shubham\Downloads\balaji udhyog-handoff\balaji-udhyog\project\assets\images\products\glassware'
...
dest_pdf = os.path.join(base_cat_dir, 'Balaji Udyog Decorative Glassware.pdf')
...
# Skip cover pages? Usually catalogues have covers.
# The user said "ye wale catalogue ki photos laga naah". We'll just extract all pages.
# Let's start from page 0 (which is 1-indexed as page_1.jpg).
start_idx = 3 # typically catalogues have covers, index 3 or 4 are products
# We will just extract all of them.
...
# Now generate Decorative Glassware.html
template = """<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width,initial-scale=1" />
<title>{title} — Balaji Udyog</title>
...
<style>
  ...
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .grid-item{{background:#fff;border:1px solid var(--line);padding:20px;text-align:center}}
...
<div class="hero">
  <div class="wrap">
    <h1>{title}</h1>
    <p style="margin-top:20px;color:rgba(255,255,255,0.8)">Explore our latest {title} collection for global export.</p>
    <div style="margin-top:30px">
      <a href="../../assets/catalogs/{pdf_filename}" target="_blank" download class="btn">
        Download Catalogue <div class="arr"></div>
      </a>
    </div>
  </div>
</div>
...
<div class="wa-cta-section wrap" style="margin: 60px auto; text-align: center; background: #25d366; padding: 50px 20px; border-radius: 12px; color: #fff;">
  ...
  <a href="https://wa.me/916290746602" target="_blank" style="display: inline-flex; align-items: center; gap: 10px; background: #fff; color: #25d366; padding: 15px 30px; border-radius: 50px; font-weight: 600; font-size: 16px; text-transform: uppercase; letter-spacing: 0.05em; text-decoration: none;">
    <svg viewBox="0 0 24 24" style="width: 24px; height: 24px; fill: currentColor;"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
    Chat with us
  </a>
</div>
</body>
...
# Let's see what start_idx is used by other categories.
# In generate_pages.py: start_idx is 3, 4, 5. 
# We'll use start_idx=3, up to max_pages
images_html = ""
for i in range(3, max_pages + 1):
    img_src = f"../../assets/images/products/glassware/page_{i}.jpg"
    images_html += f'<div class="grid-item"><img src="{img_src}" alt="Decorative Glassware Product {i}" loading="lazy"></div>\n'

html_content = template.format(title="Decorative Glassware", images_html=images_html, pdf_filename="Balaji Udyog Decorative Glassware.pdf")
out_path = r"c:\Users\Shubham\Downloads\balaji udhyog-handoff\balaji-udhyog\project\pages\products\Decorative Glassware.html"

with open(out_path, "w", encoding="utf-8") as f:
    f.write(html_content)

print(f"Generated {out_path} with {max_pages - 2} images.")
