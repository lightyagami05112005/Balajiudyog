...
# Using python format string requires escaping curly braces in css/js
# So I'll just use string replacement instead of .format() to avoid escaping hell
...
files = glob.glob(os.path.join(items_dir, "*.html"))
...
for file_path in files:
    ...
    if em:
        em.extract()
    title_text = title.get_text(strip=True).replace('<br>', '').strip()
    ...
    if sku_el:
        if '·' in sku_el:
            ...
            sku_text = sku_el.strip()
            
    sku_div = soup.find('div', class_='pdp-sku')
    if sku_div and '·' in sku_div.text:
        sku_text = sku_div.text.split('·')[1].strip()

    desc_meta = soup.find('meta', {'name': 'description'})
    desc_meta_text = desc_meta['content'] if desc_meta else title_text
    
    desc_html_el = soup.find('p', class_='pdp-lede')
    desc_html_text = desc_html_el.text.strip() if desc_html_el else desc_meta_text

    hero_img = soup.find('img', id='pdp-main-img')
    hero_img_src = hero_img['src'] if hero_img else ""
    ...
    if crumbs:
        ...
        if len(links) >= 3:
            category_text = links[2].text.strip()
            
    category_url = category_text.replace('&', 'and')

    thumbs_div = soup.find('div', class_='pdp-thumbs')
    thumbs_html = ""
    if thumbs_div:
        for t in thumbs_div.find_all('button', class_='pdp-thumb'):
            src = t.get('data-thumb', '')
            thumbs_html += f'<div class="pdp-thumb" onclick="swapImage(\'{src}\', this)"><img src="{src}" alt="Thumbnail"></div>\n'
    if not thumbs_html and hero_img_src:
        thumbs_html = f'<div class="pdp-thumb active" onclick="swapImage(\'{hero_img_src}\', this)"><img src="{hero_img_src}" alt="Thumbnail"></div>\n'

    spec_table = soup.find('div', class_='spec-table')
    specs = {}
    if spec_table:
        for r in spec_table.find_all('div', class_='r'):
            lbl = r.find('small')
            val = r.find('b')
            if lbl and val:
                specs[lbl.text.strip()] = val.text.strip()
    
    if 'OEM' not in specs and 'Customisation' not in specs:
        specs['OEM & Private Label'] = 'Available on request'
    if 'Export Quality' not in specs:
        specs['Export Quality'] = 'Premium B2B Standard'
    
    specs_html = ""
    for k, v in specs.items():
        specs_html += f'<div class="spec-card"><span class="spec-label">{k}</span><span class="spec-val">{v}</span></div>\n'
        
    related_grid = soup.find('div', class_='product-grid')
    related_html = ""
    if related_grid:
        for card in related_grid.find_all('a', class_='product-card'):
            href = card.get('href', '')
            img_tag = card.find('img')
            img_src = img_tag['src'] if img_tag else ''
            h4 = card.find('h4')
            rel_title = h4.text.strip() if h4 else ''
            sku_rel = card.find('div', class_='pc-sku')
            rel_sku_text = sku_rel.text.split('·')[0].strip() if sku_rel else ''
            
            related_html += f'''
            <a href="{href}" class="related-card">
              <div class="related-img"><img src="{img_src}" alt="{rel_title}"></div>
              <div class="related-info">
                <h4>{rel_title}</h4>
                <p>SKU: {rel_sku_text}</p>
              </div>
            </a>
            '''

    wa_text = urllib.parse.quote(f"Hello, I'm interested in {title_text} ({sku_text}). Please share quotation, MOQ and export details.")
    hero_image_clean = hero_img_src.replace('../../../', '/')

    new_content = template
    new_content = new_content.replace('__TITLE__', title_text)
    new_content = new_content.replace('__SKU__', sku_text)
    new_content = new_content.replace('__CAT__', category_text)
    new_content = new_content.replace('__CAT_URL__', category_url)
    new_content = new_content.replace('__DESC_META__', desc_meta_text.replace('"', '&quot;'))
    new_content = new_content.replace('__DESC_HTML__', desc_html_text)
    new_content = new_content.replace('__HERO__', hero_img_src)
    new_content = new_content.replace('__HERO_CLEAN__', hero_image_clean)
...
print("All PDPs updated to new B2B template.")
